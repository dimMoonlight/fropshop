...
class CountCondition(Condition):
    ...
    def consume_items(self, offer, basket, affected_lines):
        ...
        将购物篮内的物品标记为已消耗，因此无法在其他优惠中重复使用。
        ：basket ：购物篮
        ：affected_lines：受折扣影响的行。 这应该是元组列表（行，折扣，数量）
        """
        ...
        # 我们需要计算在应用效益时已经消耗了多少项目，因此我们不会消耗太多项目。
        num_consumed = 0
...
class ValueCondition(Condition):
    ...
    def consume_items(self, offer, basket, affected_lines):
        ...
        in a specific order.
        标记购物篮内的项目被消耗，因此他们不能在其他报价中重复使用。
        我们允许行传递，有时我们希望它们按特定的顺序排序。
        """
