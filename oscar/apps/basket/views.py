...
class BasketView(ModelFormSetView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # 运输信息包括给一个想法的总订单成本。对于PayPal Express来说，重要的是让客户从
        # 购物篮页面中重定向，并且需要查看预先估计的订单总数。
...
class BasketAddView(FormView):
    ...
    处理添加到购物篮的提交，这是从站点的各个部分触发的。从模块basket_tags.py.
    使用TEMPPLATEG 将添加到购物篮的形式加载到模板中。
    """
...
class VoucherRemoveView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not request.basket.id:
            ...
            # a voucher in it.
            # Hacking attempt 购物篮必须保存，以便在里面有凭证。
            return response
        try:
