...
def queryset_orders_for_user(user):
    ...
    返回允许用户访问的所有订单的查询集。
    员工用户可以访问所有订单。
    为了允许访问非工作人员用户的订单，至少有一行的合作伙伴必须在合作伙伴的列表中有用户。
    """
...
class OrderListView(BulkEditMixin, ListView):
    ...
    def get_search_filter_descriptions(self):  # noqa (too complex (19))
        ...
        这些是面向用户的消息，描述了在搜索查询中使用哪些过滤器来过滤订单。
        ...
        # 如果没有提交表单，试图从提交表单中检索数据，那么“cleaned_data(清除数据）”将
        # 不被设置，因此默认为“None”。
        ...
        if data.get('upc'):
            descriptions.append(
                ...
                # in an online store.
                # 翻译：“UPC”表示“通用产品代码”，用于唯一标识在线商店中的产品。
                # 在此上下文中的“项目”表示放置在在线商店中的订单中的项目。
        ...
        if data.get('partner_sku'):
            descriptions.append(
                ...
                # 翻译：“SKU”表示“库存单位”，用于识别可以从在线商店发货的产品。
                # “合作伙伴”是指向在线商店购物的用户发送商品的公司。
        ...
        if data.get('voucher'):
            descriptions.append(
                ...
                # 翻译：“优惠券”是可以应用于在线商店中的订单以获得折扣的优惠券。
                #  凭证“代码”是用户可以输入以接收折扣的字符串。
        ...
        if data.get('payment_method'):
            ...
            descriptions.append(
                ...
                # 翻译：付款方式是在线商店中支付商品的一种方式。
                # 例如，用户可以使用信用卡或PayPal付款。
        ...
        if data.get('status'):
            descriptions.append(
                ...
                # 译员：此字符串是指在线商店中的订单。 订单状态的一些示例是“已购买”，
                # “已取消”或“已退款”。
    ...
    def change_order_status(self, request, order):
        ...
        # 此方法与OrderDetailView.change_order_status非常相似。 成熟的重构。
...
class OrderDetailView(DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # 对于POST请求，我们使用动态调度技术，其中参数指定我们尝试使用表单提交的
        # 内容。我们区分订单级别操作和行级别操作。
