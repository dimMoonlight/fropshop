...
class ReviewListView(BulkEditMixin, generic.ListView):
    ...
    def get_date_from_to_queryset(self, date_from, date_to, queryset=None):
        ...
        获取与`date_from *和* date_to *指定的时间范围匹配的``ProductReview``项
        的``QuerySet``。 两个参数都应采用“datetime”格式，* date_from * <* date_to *。
        如果指定了* queryset *，则将根据给定日期对其进行过滤。 否则，将创建所
        有``ProductReview``项的新查询集。
        """
        if queryset is None:
    ...
    def add_filter_status(self, queryset, status):
        ...
        # evaluated to False
        # 需要检查空字符串，而不是TRUE，因为0是“状态”的有效值，但将被评估为false。
        if status != '':
    ...
    def update_selected_review_status(self, request, reviews):
        ...
        使用* request * POST数据中的新状态更新所选*评论*的状态。
         重定向回到评论的列表视图。
        """
