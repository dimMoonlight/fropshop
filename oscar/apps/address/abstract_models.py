...
class AbstractCountry(models.Model):
    ...
    国际标准化组织（ISO）3166-1国家名单。字段名称有点笨拙，但为了向后兼容而保留。
    AlpHA2、AlpH3、姓名和官名的PyFrand语法似乎是明智的。
    """
    ...
    class Meta:
        abstract = True # 摘要
        app_label = 'address' # 地址
        verbose_name = _('Country') # 地区
        verbose_name_plural = _('Countries') # 国家
        ordering = ('-display_order', 'printable_name',)
        # ordering 排序 printable_name 打印名字
    ...
    def numeric_code(self):
        ...
        速写的ISO 3166数字代码。
        ISOy31661i数值用于错误地是整数域，但必须是填充前导零点。它已经被转换成一个字符字段，
        但是数据库可能仍然包含非填充字符串。这就是为什么填料保持不变。
        """
...
class AbstractShippingAddress(AbstractAddress):
    ...
    装运地址
    一旦放置订单，则不应编辑装运地址。在那之后它应该是只读的。

    注：
    发货地址是订单应用程序的一个模型。但是移动它是棘手的由于循环导入问题，
    由GETYMID/GETH类放大调用预Django 1.7来注册接收器。所以…
    ToDO：一旦Django 1.6的支持被删除，移动ActudiBLIN地址和向订单应用程序发送SIP地址，
    然后移动合作伙伴地址的合作伙伴应用程序。
    """
...
class AbstractUserAddress(AbstractShippingAddress):
    ...
    用户的地址。用户可以有很多这些，并一起形成一个用户的“地址簿”。

    我们使用单独的航运和计费模式（即使有一些数据复制）因为我们不需要运费/帐单地址
    一旦订单已被更改或删除。通过分开模型，我们允许用户从他们的地址中添加/编辑/删除。
    没有影响订单的书已经放好了。
    """
    ...
    class Meta:
        ...
        verbose_name_plural = _("User addresses") # plural 复数的
...
class AbstractPartnerAddress(AbstractAddress):
    ...
    合作伙伴可以有一个或多个地址。这可以是有用的，例如根据货物的原产地确定美国税收。
    """
