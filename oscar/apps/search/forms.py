...
class SearchInput(Input):
    ...
    定义搜索类型小部件
    这是一个HTML5的东西，适用于Safari，其他浏览器默认使用默认的“文本”类型
    """
    input_type = 'search'
...
class SearchForm(FacetedSearchForm):
    ...
    # 将查询参数映射到排序字段。 注意此处不包括相关性，因为我们假设在没有将显式
    # 排序字段传递给搜索后端的情况下以相关性顺序返回结果。
    SORT_BY_MAP = {
        TOP_RATED: '-rating',
        NEWEST: '-date_created',
    ...
    # Non Solr后端不支持动态字段，因此我们只对标题进行排序
    ...
    def search(self):
        ...
        # 我们从FacetedSearchForm中替换'search'方法，以便我们可以处理范围查询
        # 注意，我们在父类上调用super，因为默认的分面视图会转义所有内容
        # （这对于价格范围查询不起作用）
        sqs = super(FacetedSearchForm, self).search()

        # We need to process each facet to ensure that the field name and the
        # value are quoted correctly and separately:
        # 我们需要处理每个方面，以确保正确和单独引用字段名称和值：
        for field, values in self.selected_multi_facets.items():
            ...
            if field in VALID_FACET_QUERIES:
                ...
                # 查询方面 - 不要在语音标记中包装值，也不要清除值。 此点应该验证查询值，因此我们不需要转义它们。
                sqs = sqs.narrow('%s:(%s)' % (
...
class BrowseCategoryForm(SearchForm):
    ...
    query is specified.
    SearchForm的变体，如果未指定查询，则返回所有产品（而不是无）。
    """
