...
class KeywordPromotion(LinkedPromotion):
    ...
    # 我们允许使用额外的过滤器，以便将搜索查询匹配限制在站点的不同部分。
...
class RawHTML(AbstractPromotion):
    ...
    # required.
    # 用于确定如何呈现促销（例如，如果需要不同宽度的容器）。 这并不总是必需的。
...
class Image(AbstractPromotion):
    ...
    图像促销只是一个命名图像，它具有到站点（或另一个站点）另一部分的可选链接。
    这可用于模拟横幅和广告连播。
    """
    _type = 'Image'
    name = models.CharField(_("Name"), max_length=128)
...
class MultiImage(AbstractPromotion):
    ...
    rotating banners.
    多图像促销只是以特定方式呈现的图像促销的集合。 这可以模拟旋转横幅等内容。
    """
    _type = 'Multi-image'
    name = models.CharField(_("Name"), max_length=128)
